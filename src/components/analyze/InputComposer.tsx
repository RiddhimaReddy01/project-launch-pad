  ...
  return (
    ...
    >
      ...
      <div className="flex items-center justify-between p-4" style={{ borderBottom: '1px solid var(--divider)' }}>
        <div>
          <p style={{ fontSize: 13, fontWeight: 500, letterSpacing: '0.06em', textTransform: 'uppercase', color: 'var(--text-muted)' }}>
            Input Composer
          </p>
        </div>
        <button
      ...
      <div className="flex-1 overflow-y-auto p-4" style={{ fontSize: 13 }}>
        {/* Presets */}
        <p style={{ fontSize: 13, fontWeight: 500, letterSpacing: '0.06em', textTransform: 'uppercase', color: 'var(--text-muted)', marginBottom: 8 }}>
          Presets
        </p>
          ...
          {INPUT_ITEMS.map(item => (
            ...
            >
              ...
              <div className="flex-1 min-w-0">
                <span style={{ fontSize: 13, fontWeight: inputs[item.key] ? 500 : 400, color: inputs[item.key] ? 'var(--text-primary)' : 'var(--text-secondary)', display: 'block' }}>
                  {item.label}
                </span>
                {item.value && inputs[item.key] && (
                  <span style={{ fontSize: 13, color: 'var(--text-muted)', display: 'block', marginTop: 1, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
                    {item.value}
                  </span>
                )}
              </div>
              ...
              {PRIOR_SECTIONS.map(s => {
                ...
                return (
                  ...
                  >
                    ...
                    <span style={{ fontSize: 13, fontWeight: included ? 500 : 400, color: included ? 'var(--text-primary)' : 'var(--text-secondary)' }}>{s.label}</span>
                  </label>
                );
              })}
            </div>
        ...
        <div className="rounded-[8px] p-3 mb-4" style={{ backgroundColor: 'var(--surface-card)', border: '1px solid var(--divider)' }}>
          <p style={{ fontSize: 13, fontWeight: 400, color: 'var(--text-primary)', marginBottom: 2 }}>
