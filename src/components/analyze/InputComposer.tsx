  ...
  return (
    ...
    >
      ...
      <div className="flex-1 overflow-y-auto p-4" style={{ fontSize: 13 }}>
        {/* Presets */}
        <p style={{ fontFamily: "'Outfit', sans-serif", fontSize: 10, fontWeight: 500, letterSpacing: '0.06em', textTransform: 'uppercase', color: 'var(--text-muted)', marginBottom: 8 }}>
          Presets
        </p>
          ...
          {INPUT_ITEMS.map(item => (
            ...
            >
              ...
              <div className="flex-1 min-w-0">
                <span style={{ fontFamily: "'Outfit', sans-serif", fontSize: 12, fontWeight: inputs[item.key] ? 400 : 300, color: inputs[item.key] ? 'var(--text-primary)' : 'var(--text-secondary)', display: 'block' }}>
                  {item.label}
                </span>
                {item.value && inputs[item.key] && (
                  <span style={{ fontFamily: "'Outfit', sans-serif", fontSize: 10, color: 'var(--text-muted)', display: 'block', marginTop: 1, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
                    {item.value}
                  </span>
                )}
              </div>
              ...
              {PRIOR_SECTIONS.map(s => {
                ...
                return (
                  ...
                  >
                    ...
                    <span style={{ fontFamily: "'Outfit', sans-serif", fontSize: 12, fontWeight: included ? 400 : 300, color: included ? 'var(--text-primary)' : 'var(--text-secondary)' }}>{s.label}</span>
                  </label>
                );
              })}
            </div>
        ...
        <div className="rounded-[8px] p-3 mb-4" style={{ backgroundColor: 'var(--surface-card)', border: '1px solid var(--divider)' }}>
          <p style={{ fontFamily: "'Outfit', sans-serif", fontSize: 11, fontWeight: 400, color: 'var(--text-primary)', marginBottom: 2 }}>
            Using {inputCount} input{inputCount !== 1 ? 's' : ''}
          </p>
          <p style={{ fontFamily: "'Outfit', sans-serif", fontSize: 10, color: 'var(--text-muted)', lineHeight: 1.4 }}>
            {[
        ...
        {isDirty && hasResult && (
          <div className="rounded-[8px] p-3 mb-4" style={{ backgroundColor: 'rgba(212,136,15,0.06)', border: '1px solid rgba(212,136,15,0.15)' }}>
            <p style={{ fontFamily: "'Outfit', sans-serif", fontSize: 11, color: 'var(--accent-amber)' }}>
      ...
      <div className="p-4" style={{ borderTop: '1px solid var(--divider)' }}>
        ...
        {hasResult && !isDirty && (
          <p style={{ fontFamily: "'Outfit', sans-serif", fontSize: 10, color: 'var(--text-muted)', textAlign: 'center', marginTop: 6 }}>
            Up to date
          </p>
        )}
      </div>
