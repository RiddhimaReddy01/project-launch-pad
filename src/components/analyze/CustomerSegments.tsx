      ...
      <div className="flex flex-col lg:flex-row gap-6">
        ...
        <div className="flex-1">
          <div className="rounded-[12px] p-6" style={{ backgroundColor: 'var(--surface-card)', border: '1px solid var(--divider)' }}>
            <p style={{ fontFamily: "'Inter', sans-serif", fontSize: 18, fontWeight: 400, color: 'var(--text-primary)', marginBottom: 2 }}>{seg.name}</p>
            <p style={{ fontFamily: "'Inter', sans-serif", fontSize: 12, color: 'var(--text-muted)', marginBottom: 16 }}>~{fmtSize(seg.estimated_size)} people locally</p>
            ...
            {/* Pain bar */}
            <div className="mb-5">
              <p style={{ fontFamily: "'Inter', sans-serif", fontSize: 10, fontWeight: 500, letterSpacing: '0.08em', textTransform: 'uppercase', color: 'var(--text-muted)', marginBottom: 6 }}>Pain Intensity</p>
              <div className="flex items-center gap-1">
                ...
                <span style={{ fontFamily: "'Inter', sans-serif", fontSize: 12, fontWeight: 400, color: 'var(--text-primary)', marginLeft: 8 }}>{seg.pain_intensity}/10</span>
              </div>
            </div>
            ...
            {[
              ...
              <div key={item.label} className="mb-4">
                <p style={{ fontFamily: "'Inter', sans-serif", fontSize: 10, fontWeight: 500, letterSpacing: '0.08em', textTransform: 'uppercase', color: 'var(--text-muted)', marginBottom: 6 }}>{item.label}</p>
                {item.pill ? (
                  <span className="rounded-full px-3 py-1" style={{ fontSize: 12, fontFamily: "'Inter', sans-serif", backgroundColor: 'rgba(26,26,26,0.03)', color: 'var(--text-primary)', border: '1px solid var(--divider)' }}>{item.value}</span>
                ) : (
                  <p style={{ fontFamily: "'Inter', sans-serif", fontSize: 13, fontWeight: 300, color: 'var(--text-secondary)', lineHeight: 1.5 }}>{item.value}</p>
                )}
              </div>
            ))}
          </div>
        ...
        <div className="lg:w-[280px] flex-shrink-0 flex flex-col gap-3">
          <div className="rounded-[12px] p-4" style={{ backgroundColor: 'var(--surface-card)', border: '1px solid var(--divider)' }}>
            <p style={{ fontFamily: "'Inter', sans-serif", fontSize: 10, fontWeight: 500, letterSpacing: '0.08em', textTransform: 'uppercase', color: 'var(--text-muted)', marginBottom: 8, textAlign: 'center' }}>Segment Profile</p>
          ...
          <div className="rounded-[12px] p-4" style={{ backgroundColor: 'var(--surface-card)', border: '1px solid var(--divider)' }}>
            <p style={{ fontFamily: "'Inter', sans-serif", fontSize: 10, fontWeight: 500, letterSpacing: '0.08em', textTransform: 'uppercase', color: 'var(--text-muted)', marginBottom: 8 }}>All Segments</p>
            <div className="flex flex-col gap-1.5">
              {data.segments.map((s, i) => (
                <div
                  ...
                  onClick={() => setSelected(i)}
                >
                  <span style={{ fontFamily: "'Inter', sans-serif", fontSize: 12, fontWeight: selected === i ? 400 : 300, color: selected === i ? 'var(--text-primary)' : 'var(--text-muted)' }}>{s.name}</span>
