    ...
    <div>
      ...
      <div className="rounded-[12px] p-6 mb-8 flex items-center gap-6" style={{ backgroundColor: 'var(--surface-card)', border: '1px solid var(--divider)' }}>
        <div style={{ width: 64, height: 64, borderRadius: '50%', border: `3px solid ${scoreColor}`, display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
          <span style={{ fontSize: 22, fontWeight: 400, color: scoreColor }}>{data.score}</span>
        </div>
        <div>
          <p style={{ fontSize: 13, fontWeight: 500, letterSpacing: '0.08em', textTransform: 'uppercase', color: scoreColor, marginBottom: 4 }}>Location Score</p>
          <p style={{ fontSize: 14, fontWeight: 400, color: 'var(--text-secondary)', lineHeight: 1.6 }}>{data.verdict}</p>
        </div>
      </div>
      ...
      {data.city_center && data.focus_areas && data.focus_areas.length > 0 && (
        <div className="mb-8">
          <p className="font-caption" style={{ fontSize: 13, letterSpacing: '0.08em', textTransform: 'uppercase', marginBottom: 12 }}>Location Map</p>
      ...
      {/* Demographics grid */}
      <div className="mb-8">
        <p className="font-caption" style={{ fontSize: 13, letterSpacing: '0.08em', textTransform: 'uppercase', marginBottom: 12 }}>Demographics</p>
      ...
      {/* Foot traffic */}
      <div className="mb-8">
        <p className="font-caption" style={{ fontSize: 13, letterSpacing: '0.08em', textTransform: 'uppercase', marginBottom: 12 }}>Foot Traffic & Real Estate</p>
        <div className="rounded-[12px] p-5" style={{ backgroundColor: 'var(--surface-card)', border: '1px solid var(--divider)' }}>
          <div className="flex flex-col gap-4">
            <div>
              <p style={{ fontSize: 13, fontWeight: 500, letterSpacing: '0.06em', textTransform: 'uppercase', color: 'var(--text-muted)', marginBottom: 6 }}>Best Areas</p>
              <div className="flex flex-wrap gap-2">
                {data.foot_traffic.best_areas.map((area, i) => (
                  <span key={i} className="rounded-full px-3 py-1" style={{ fontSize: 13, backgroundColor: 'rgba(26,26,26,0.03)', color: 'var(--text-primary)', border: '1px solid var(--divider)' }}>{area}</span>
                ))}
              </div>
            </div>
            <div className="flex gap-8">
              <div>
                <p style={{ fontSize: 13, fontWeight: 500, letterSpacing: '0.06em', textTransform: 'uppercase', color: 'var(--text-muted)', marginBottom: 4 }}>Avg Rent /sqft</p>
                <p style={{ fontSize: 18, fontWeight: 400, color: 'var(--text-primary)' }}>${data.foot_traffic.avg_monthly_rent_sqft}</p>
              </div>
              <div>
                <p style={{ fontSize: 13, fontWeight: 500, letterSpacing: '0.06em', textTransform: 'uppercase', color: 'var(--text-muted)', marginBottom: 4 }}>Competitor Density</p>
                <p style={{ fontSize: 18, fontWeight: 400, color: 'var(--text-primary)' }}>{data.foot_traffic.competitor_density}</p>
              </div>
            </div>
          </div>
        </div>
      ...
      {/* Regulatory */}
      <div>
        <p className="font-caption" style={{ fontSize: 13, letterSpacing: '0.08em', textTransform: 'uppercase', marginBottom: 12 }}>Regulatory & Permits</p>
        <div className="rounded-[12px] p-5" style={{ backgroundColor: 'var(--surface-card)', border: '1px solid var(--divider)' }}>
          <div className="flex flex-col gap-3">
            <div>
              <p style={{ fontSize: 13, fontWeight: 500, letterSpacing: '0.06em', textTransform: 'uppercase', color: 'var(--text-muted)', marginBottom: 6 }}>Key Permits Required</p>
              <div className="flex flex-col gap-1.5">
                {data.regulatory.key_permits.map((permit, i) => (
                  <div key={i} className="flex items-center gap-2">
                    ...
                    <span style={{ fontSize: 13, fontWeight: 400, color: 'var(--text-secondary)' }}>{permit}</span>
                  </div>
                ))}
              </div>
            </div>
            <div className="flex gap-8 pt-2" style={{ borderTop: '1px solid var(--divider)' }}>
              <div>
                <p style={{ fontSize: 13, fontWeight: 500, letterSpacing: '0.06em', textTransform: 'uppercase', color: 'var(--text-muted)', marginBottom: 4 }}>Timeline</p>
                <p style={{ fontSize: 14, fontWeight: 400, color: 'var(--text-primary)' }}>{data.regulatory.estimated_timeline}</p>
              </div>
            </div>
            {data.regulatory.notes && (
              <p style={{ fontSize: 13, fontWeight: 400, color: 'var(--text-muted)', lineHeight: 1.5, marginTop: 4 }}>{data.regulatory.notes}</p>
            )}
          </div>
        </div>
      </div>
...
function StatCard({ label, value }: { label: string; value: string }) {
  return (
    <div className="rounded-[10px] p-4" style={{ backgroundColor: 'var(--surface-card)', border: '1px solid var(--divider)' }}>
      <p style={{ fontSize: 13, fontWeight: 500, letterSpacing: '0.06em', textTransform: 'uppercase', color: 'var(--text-muted)', marginBottom: 6 }}>{label}</p>
      <p style={{ fontSize: 20, fontWeight: 400, color: 'var(--text-primary)' }}>{value}</p>
    </div>
  );
}
