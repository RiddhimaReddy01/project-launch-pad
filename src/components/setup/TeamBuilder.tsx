    ...
    <div>
      ...
      <div className="grid grid-cols-3 gap-3 mb-8">
        <div className="rounded-[10px] p-4" style={{ backgroundColor: 'var(--surface-card)', border: '1px solid var(--divider)' }}>
          <p style={{ fontSize: 13, fontWeight: 500, letterSpacing: '0.06em', textTransform: 'uppercase', color: 'var(--text-muted)', marginBottom: 6 }}>Total Payroll</p>
          <p style={{ fontSize: 20, fontWeight: 400, color: 'var(--text-primary)' }}>{fmt(data.total_payroll)}</p>
        </div>
        <div className="rounded-[10px] p-4" style={{ backgroundColor: 'var(--surface-card)', border: '1px solid var(--divider)' }}>
          <p style={{ fontSize: 13, fontWeight: 500, letterSpacing: '0.06em', textTransform: 'uppercase', color: 'var(--text-muted)', marginBottom: 6 }}>Must Have</p>
          <p style={{ fontSize: 20, fontWeight: 400, color: 'var(--text-primary)' }}>{fmt(mustHaveCost)}</p>
        </div>
        <div className="rounded-[10px] p-4" style={{ backgroundColor: 'var(--surface-card)', border: '1px solid var(--divider)' }}>
          <p style={{ fontSize: 13, fontWeight: 500, letterSpacing: '0.06em', textTransform: 'uppercase', color: 'var(--text-muted)', marginBottom: 6 }}>Nice to Have</p>
      ...
      <div className="mb-8">
        <p style={{ fontSize: 13, fontWeight: 500, letterSpacing: '0.08em', textTransform: 'uppercase', color: 'var(--text-muted)', marginBottom: 12 }}>
          Hiring Timeline
        </p>
            ...
            return (
              <div key={i} className="flex items-center mb-1.5" style={{ height: 28 }}>
                <div style={{ width: 120, flexShrink: 0 }}>
                  <p style={{ fontSize: 13, fontWeight: 400, color: 'var(--text-primary)', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
                    {member.title}
                  </p>
                </div>
                <div className="flex-1 relative" style={{ height: 20 }}>
                  <div style={{ position: 'absolute', top: 0, bottom: 0, left: `${startPct}%`, width: `${widthPct}%`, borderRadius: 4, backgroundColor: typeStyle.bg, border: `1px solid ${typeStyle.color}30` }}>
                    <span style={{ fontSize: 13, color: typeStyle.color, paddingLeft: 6, lineHeight: '18px' }}>
                      {member.salary_label}
                    </span>
                  </div>
                </div>
            ...
            <div key={i} className="rounded-[12px] p-5" style={{ backgroundColor: 'var(--surface-card)', border: '1px solid var(--divider)' }}>
              <div className="flex items-start justify-between gap-3 mb-2">
                <div className="flex items-center gap-2 flex-wrap">
                  <span style={{ fontSize: 14, fontWeight: 400, color: 'var(--text-primary)' }}>{member.title}</span>
                ...
                <span style={{ fontSize: 13, fontWeight: 400, color: 'var(--text-muted)', whiteSpace: 'nowrap' }}>Month {member.month}</span>
              </div>
              <p className="font-heading" style={{ fontSize: 18, marginBottom: 6 }}>{member.salary_label}</p>
              <p style={{ fontSize: 13, fontWeight: 400, color: 'var(--text-secondary)', lineHeight: 1.6 }}>{member.why_needed}</p>
            </div>
          );
        })}
      </div>
