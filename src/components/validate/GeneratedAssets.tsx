  ...
  return (
    ...
    >
      <div className="flex items-center justify-between" style={{ marginBottom: 20 }}>
        <span style={{ fontSize: 15, fontWeight: 400, color: 'var(--text-primary)' }}>{title}</span>
        <div className="flex items-center" style={{ gap: 12 }}>
          {copyText && <CopyButton text={copyText} />}
          <button style={{ fontSize: 13, fontWeight: 400, color: 'var(--text-muted)', background: 'none', border: 'none', cursor: 'pointer' }}>
            Regenerate
          </button>
        </div>
      </div>
    ...
    <SectionBlock title="Landing page preview" copyText={allText}>
      <div style={{ borderRadius: 12, border: '1px solid var(--divider-light)', overflow: 'hidden' }}>
        <div style={{ padding: '40px 32px', textAlign: 'center', backgroundColor: 'var(--surface-bg)' }}>
          ...
          <p style={{ fontSize: 15, fontWeight: 400, color: 'var(--text-secondary)', marginBottom: 28 }}>{lp.subheadline}</p>
          <div style={{ maxWidth: 340, margin: '0 auto', textAlign: 'left', marginBottom: 28 }}>
            {lp.benefits.map((b, i) => (
              <div key={i} className="flex items-start" style={{ gap: 10, marginBottom: 10 }}>
                ...
                <span style={{ fontSize: 13, fontWeight: 400, color: 'var(--text-secondary)', lineHeight: 1.6 }}>{b}</span>
              </div>
            ))}
          </div>
          <button style={{
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
        {MOCK_SURVEY.map((q, i) => (
          <div key={q.id} style={{ padding: '16px 20px', borderRadius: 10, backgroundColor: 'var(--surface-bg)' }}>
            <p style={{ fontSize: 13, fontWeight: 400, color: 'var(--text-primary)', marginBottom: q.options ? 10 : 0 }}>
            ...
            {q.type === 'email' && (
              <div style={{ marginTop: 8, padding: '8px 12px', borderRadius: 8, border: '1px solid var(--divider-light)', backgroundColor: '#FFFFFF' }}>
                <span style={{ fontSize: 13, color: 'var(--text-muted)' }}>email@example.com</span>
              </div>
            )}
          </div>
        ))}
  ...
  return (
    <SectionBlock title="Social outreach message" copyText={so.message}>
      <div style={{ padding: 20, borderRadius: 14, backgroundColor: 'var(--surface-bg)', position: 'relative' }}>
        <span style={{ fontSize: 13, fontWeight: 400, color: 'var(--text-muted)', marginBottom: 8, display: 'block' }}>
          Tone: {so.tone}
        </span>
        <div style={{
          ...
          <p style={{ fontSize: 14, fontWeight: 400, color: 'var(--text-secondary)', lineHeight: 1.75, whiteSpace: 'pre-line' }}>
            {so.message}
          </p>
        </div>
      </div>
  ...
  return (
    <SectionBlock title="Marketplace listing" copyText={allText}>
      <div style={{ padding: 20, borderRadius: 12, border: '1px solid var(--divider-light)' }}>
        <p style={{ fontSize: 14, fontWeight: 400, color: 'var(--accent-primary)', marginBottom: 8 }}>{ml.hook}</p>
        <p style={{ fontSize: 15, fontWeight: 400, color: 'var(--text-primary)', marginBottom: 12 }}>{ml.title}</p>
        <p style={{ fontSize: 13, fontWeight: 400, color: 'var(--text-secondary)', lineHeight: 1.75, marginBottom: 16 }}>{ml.description}</p>
        <span style={{
          fontSize: 13, fontWeight: 400,
    ...
    <SectionBlock title="Direct outreach kit" copyText={allText}>
      {[
        ...
        <div key={item.label} style={{ marginBottom: 16, padding: '16px 20px', borderRadius: 10, backgroundColor: 'var(--surface-bg)' }}>
          <span style={{ fontSize: 13, fontWeight: 400, color: 'var(--text-muted)', textTransform: 'uppercase', letterSpacing: '0.05em', display: 'block', marginBottom: 8 }}>
            {item.label}
          </span>
          <p style={{ fontSize: 14, fontWeight: 400, color: 'var(--text-secondary)', lineHeight: 1.75 }}>{item.text}</p>
        </div>
      ))}
    </SectionBlock>
  );
  ...
  if (methods.length === 0) {
    return (
      <div style={{ textAlign: 'center', padding: '60px 0' }}>
        <p style={{ fontSize: 15, fontWeight: 400, color: 'var(--text-muted)' }}>
